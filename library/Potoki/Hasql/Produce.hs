...
statefulSession session initialState =
  havingConnection $ \ connection -> do
    ...
    return $ A.Fetch $ \_ yield -> do
      state <- readIORef stateRef
      sessionResult <- G.run (session state) connection
      case sessionResult of
        Left error -> return (yield (Left (I.sessionError error)))
        Right (result, newState) -> do
          writeIORef stateRef newState
          return (yield (Right result))
  ...
  Produce $ do
    ...
    case errorOrConnection of
      Left error ->
        let
          fetch =
            A.Fetch $ \_ yield -> return (yield (Left (I.connectionError error)))
          kill =
            return ()
